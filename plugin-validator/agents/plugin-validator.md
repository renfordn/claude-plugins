...
- Explicit request: "validate my plugin", "check plugin structure before I publish", "did I set up plugin.json right?"
- Proactively, right after a plugin component is created or modified: a new command, agent, skill, or hook file, or an edit to `.claude-plugin/plugin.json`. Don't wait to be asked — validate, then report.
...
5. **Validate Agents** (if `agents/` exists):
   ...
   - For each agent file, check:
     ...
     - The YAML frontmatter actually parses — this is the single most common failure. A `description` that embeds literal `<example>`/`user:`/`assistant:` blocks at column 0, unindented and unquoted, breaks the YAML parser because those lines look like new top-level keys. Either keep `description` a single-line plain scalar, or use a block scalar (`description: |-`) with every continuation line indented under the key.
     - System prompt (the markdown body) exists and is substantial (>20 chars)
...
7. **Validate Hooks** (if `hooks/hooks.json` exists):
   ...
   - Commands reference existing scripts via `${CLAUDE_PLUGIN_ROOT}`
