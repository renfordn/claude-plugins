...
2. The re-spawn message carries the prior report's context
...
class BeforeContinueModelEscalationTests(unittest.TestCase):
    ...
    def test_escalation_message_instructs_respawn_with_prior_context(self):
        """
        Test: systemMessage instructs re-spawning agent-TDD at the escalated
        tier with the prior report's findings (the hook can't invoke the
        Agent tool itself).
        """
        with h.temp_git_repo() as repo, h.temp_home() as home:
            feature_dir = h.feature_spec_dir(home, repo)
            ...
            self.assertIn("report's findings", result)
