...
`spec-reviewer` has no "File Summaries" output of its own, unlike `research-consolidator` —
this is deliberate, not an oversight. `research-consolidator` researches the codebase and
routinely surfaces durable, project-level facts (an interface, a constraint) independent of the
feature being planned. `spec-reviewer` reviews caller-supplied requirement text for this one
feature; its output (gaps, rewritten EARS sections) is almost always feature-scoped rather than
durable across features. The rare exception (e.g. a source ticket assumes an interface that's
since been deprecated project-wide) is still caught by the `after-requirements` hook's generic
