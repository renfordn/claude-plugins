...
Delegate entirely to agent-nelly's brief-assembly contract: ask
agent-nelly for a brief given this project's `cwd` and the task
description. Return agent-nelly's four sections verbatim, in order —
`Intent`, `Relevant entries`, `Intent alignment`, `Written`.

Whenever a task description was actually supplied (i.e. `/nelly-memory view
<task description>`), also pass agent-nelly the surface-relevant-memory
...
surfacing (agent-nelly's proactive-surfacing behavior only ever fires
...
flag — agent-nelly has nothing to match relevance against, and should
...
alongside the task description, pass those along to agent-nelly as
`target files`. When agent-nelly finds a match, its returned
...
agent-nelly produced it, never as a separate top-level section. This
...
has no direct visibility into `entries/*.md`; agent-nelly alone reads
those files and returns only the condensed brief.

If agent-nelly reports that the project has no `Intent` captured and
...
Ask agent-nelly to record `<description>` as its `error lesson`
input, exactly as documented under "Recording an error lesson" in
`agents/agent-nelly.md` — agent-nelly writes it as a new
...
input and agent-nelly performs the write.

Report agent-nelly's reply verbatim (including the `Written` line
...
Ask agent-nelly to run its `confirm error lesson: <name>` input,
exactly as documented under "Confirming an inferred error lesson" in
`agents/agent-nelly.md` — this is the only mechanism that flips an
...
brief input and agent-nelly performs the write.
...
Ask agent-nelly to run its `list inferred lessons` input, exactly as
documented under "Listing inferred error lessons" in
`agents/agent-nelly.md`. This is read-only — it never confirms,
discards, or otherwise mutates anything.

Report the returned numbered list verbatim: one line per entry, `<n>. <name>
— <description> (last written: <date>)`. If agent-nelly reports no
...
1. Run `list-inferred` above (same agent-nelly call, same output
...
2. Otherwise, show the numbered list to the user and ask them to choose
   ...
   from the list alone.
3. For every number the user marks to confirm, ask agent-nelly to
   run `confirm error lesson: <name>` (the same action `confirm-lesson`
   above uses) for that entry's `<name>` — one agent-nelly call per
   confirmed entry.
4. For every number the user marks to discard, ask agent-nelly to
   run `discard error lesson: <name>`, documented under "Discarding an
   inferred error lesson" in `agents/agent-nelly.md` — one
   agent-nelly call per discarded entry. This archives the entry
...
every confirm and every discard above is agent-nelly's action, asked
