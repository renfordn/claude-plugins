...
class TestHookTelemetryLogger(unittest.TestCase):
    ...
    @patch.dict(os.environ, {"PLUGIN_HARNESS_TELEMETRY": "off"}, clear=True)
...
class TestStandaloneTelemetrySink(unittest.TestCase):
    """Slice 6: get_hook_telemetry_logger(None, cwd=...) must construct a
    working (non-no-op) sink at a project-slug-keyed path under the harness's
    own plugin data dir, instead of silently no-op'ing standalone calls."""

    def test_standalone_cwd_produces_a_working_sink_not_a_no_op(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch.dict(os.environ, {"CLAUDE_PLUGIN_DATA": tmp}):
                logger = get_hook_telemetry_logger(None, cwd="/some/project")
                logger.emit("standalone_context_requested", agent_type="agent-tdd")

            hooks_dir = str(Path(__file__).parent.parent / "hooks")
            if hooks_dir not in sys.path:
                sys.path.insert(0, hooks_dir)
            from hook_state import project_slug  # noqa: E402 (test-only import)
            slug = project_slug("/some/project")
            log_path = Path(tmp) / slug / "telemetry.jsonl"
            records = _read_jsonl(log_path)
            self.assertEqual(len(records), 1)
            self.assertEqual(records[0]["event_type"], "standalone_context_requested")

    def test_no_workflow_state_dir_and_no_cwd_is_still_a_safe_no_op(self):
        logger = get_hook_telemetry_logger(None)
        logger.emit("hook_invoked", hook="before_continue")  # must not raise
