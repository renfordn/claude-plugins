...
try:
    ...
    # Broad catch (not just ImportError): some environments have `mcp`
    # installed but broken at import time (e.g. an incompatible
    # pydantic/CPython combination raises TypeError deep inside `mcp`'s own
    # model construction, not ImportError) -- either way this whole
    # subprocess-based suite can't run, so skip it the same way as if `mcp`
    # were simply absent. See recap.md for this feature's tracked
