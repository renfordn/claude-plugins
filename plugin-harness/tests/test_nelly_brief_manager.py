...
class TestNellyBriefManagerCacheBackend(unittest.TestCase):
    """Test cache_backend routing: exactly one of workflow_state/cache_backend is used per call.

    Slice 4 (Plugin-Harness -> Plugin-Harness Rework): fetch_brief() gains an optional
    cache_backend param. When provided, all cache reads/writes route through it instead of
    workflow_state["orchestration"]["nelly_brief_cache"]. When omitted (default None), behavior
    must be byte-for-byte identical to today's workflow_state-only caching.
    """

    def test_fetch_brief_uses_cache_backend_when_provided(self):
        """WHEN a cache_backend is passed, fetch_brief reads/writes through it and never
        touches workflow_state's nelly_brief_cache dict in that call."""
        manager = NellyBriefManager()
        mock_backend = MagicMock()
        mock_backend.get.return_value = None  # cache miss -> triggers a fresh fetch

        workflow_state = {"orchestration": {"nelly_brief_cache": {}}}
        mock_brief_text = "## Intent\nGoal\n## Relevant Entries\nEntries"
        mock_metadata = {"task_id": "test_cache_backend", "duration": 0.1}
        ...
        # cache_backend must be used for both the lookup and the write-back
        mock_backend.get.assert_called()
        mock_backend.set.assert_called()

        # workflow_state's own cache dict must be left completely untouched
        self.assertEqual(workflow_state["orchestration"]["nelly_brief_cache"], {})

    def test_fetch_brief_uses_workflow_state_only_when_no_cache_backend(self):
        """WHEN cache_backend is omitted (default None), workflow_state's cache dict is used
        exactly as before, and no cache_backend object is ever touched (none is passed)."""
        manager = NellyBriefManager()
        workflow_state = {"orchestration": {"nelly_brief_cache": {}}}
        mock_brief_text = "## Intent\nGoal\n## Relevant Entries\nEntries"
        mock_metadata = {"task_id": "test_no_cache_backend", "duration": 0.1}
        ...
        cache = workflow_state["orchestration"]["nelly_brief_cache"]
        self.assertEqual(cache.get("brief_text"), mock_brief_text)
        self.assertEqual(cache.get("metadata"), mock_metadata)
