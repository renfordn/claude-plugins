...
from orchestrator.nelly import NellyBriefManager, NELLY_BRIEF_CACHE_TTL
...
from orchestrator.harness_context_cache import HarnessContextCache
...
# project_slug() is needed here, to scope the standalone spawn path's
# HarnessContextCache to the calling project without requiring
# workflow-state.json to exist.
...
def handle_standalone_spawn(agent_type: str, cwd: str) -> dict:
    """Workflow-agnostic Tier-1-equivalent context for callers with no active
    agent-isdd SDD workflow (i.e. workflow_state_path(cwd) is None).

    Builds the same capability map Tier 1 already computes (pure filesystem
    read, no workflow_state dependency), plus a standalone nelly-brief lookup
    backed by HarnessContextCache instead of
    workflow_state["orchestration"]["nelly_brief_cache"], and emits exactly
    one "standalone_context_requested" telemetry event. Never creates,
    reads, or mutates workflow-state.json, checkpoints, or rollback state.

    Returns a strict subset of handle_agent_spawn's last_spawn_context shape
    -- {"capability_map": ..., "nelly_brief": ..., "source": "standalone"} --
    so get_spawn_context (Slice 7) can format both shapes behind one
    `source` field check rather than two divergent formatters.

    Args:
        agent_type: Name of agent being spawned (e.g., "agent-tdd")
        cwd: Current working directory (used to scope the HarnessContextCache
            to this project via project_slug(cwd))

    Returns:
        {"capability_map": dict, "nelly_brief": str | None, "source": "standalone"}
    """
    capability_map = _build_capability_map()

    cache = HarnessContextCache(project_slug(cwd))
    # Cache the capability map too (not just the nelly brief), so a
    # read-only caller like get_spawn_context (Slice 7) can retrieve it
    # later without re-parsing INTEROP.md files itself -- design.md's
    # HarnessContextCache backs "both the capability-map cache and a
    # nelly-brief cache for standalone callers".
    cache.set("capability_map", capability_map, NELLY_BRIEF_CACHE_TTL)

    manager = NellyBriefManager()
    try:
        brief_text, _metadata = manager.fetch_brief(
            cwd=cwd,
            task_description=f"standalone spawn: {agent_type}",
            workflow_state=None,
            cache_backend=cache,
        )
    except Exception as e:
        # Graceful degradation, matching _fetch_nelly_brief's own catch-all:
        # agent-nelly being absent/unreachable must never block a standalone
        # spawn.
        logger.warning(
            f"Standalone nelly brief fetch failed: {e}. Continuing without brief."
        )
        brief_text = None

    telemetry = get_hook_telemetry_logger(None, cwd=cwd)
    telemetry.emit("standalone_context_requested", agent_type=agent_type)

    return {
        "capability_map": capability_map,
        "nelly_brief": brief_text,
        "source": "standalone",
    }
