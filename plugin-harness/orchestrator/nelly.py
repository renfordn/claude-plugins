...
class NellyBriefManager:
    ...
    def fetch_brief(
        ...
        in workflow_state["orchestration"]["nelly_brief_cache"] -- or, when
        cache_backend is provided, in cache_backend instead (see below).
        ...
        Args:
            ...
            workflow_state: Workflow state dict. Ignored for caching purposes
                when cache_backend is provided (still passed through to
                _call_agent_nelly's pending-request queue).
            cache_backend: Optional HarnessContextCache (or duck-typed
                get(key)/set(key, value, ttl_seconds) object). When provided,
                all cache reads/writes route through it instead of
                workflow_state["orchestration"]["nelly_brief_cache"] -- the
                two are mutually exclusive per call (see Slice 4 / design.md's
                Data Contracts And Interfaces invariant: exactly one cache
                backend drives caching per call, never both).
        ...
        if cache_backend is not None:
            return self._fetch_brief_via_cache_backend(
                cwd, task_description, cache_backend, intent_hash, design_hash,
                workflow_state or {},
            )
    ...
    _CACHE_BACKEND_KEY = "nelly_brief"

    def _fetch_brief_via_cache_backend(
        self,
        cwd: str,
        task_description: str,
        cache_backend,
        intent_hash: str,
        design_hash: str,
        workflow_state_for_pending_queue: dict,
    ) -> Tuple[Optional[str], dict]:
        """cache_backend-routed counterpart of fetch_brief's workflow_state
        path: same hash/TTL invalidation semantics, same graceful-degradation
        fallback to stale cache on fetch failure, but reads/writes go through
        cache_backend.get(key)/set(key, value, ttl_seconds) instead of
        mutating workflow_state["orchestration"]["nelly_brief_cache"].
        """
        cached = cache_backend.get(self._CACHE_BACKEND_KEY)
        if (
            cached
            and cached.get("intent_hash") == intent_hash
            and cached.get("design_hash") == design_hash
        ):
            return cached.get("brief_text"), cached.get("metadata", {})

        try:
            brief_text, metadata = self._call_agent_nelly(
                cwd, task_description, intent_hash, design_hash,
                workflow_state_for_pending_queue,
            )

            cache_backend.set(
                self._CACHE_BACKEND_KEY,
                {
            ...
            stale = cache_backend.get(self._CACHE_BACKEND_KEY)
            if stale and stale.get("brief_text") is not None:
                logger.warning(
                    "Using stale nelly brief cache due to fetch failure. "
                    "Brief may be out of date."
                )
                return stale.get("brief_text"), stale.get("metadata", {})
