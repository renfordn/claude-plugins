...
class TestGetSiblingPluginDataDir(unittest.TestCase):
    ...
    argument and returned plugin-orchestrator's own directory instead of
    ...
    def test_marketplace_suffix_is_preserved_for_sibling(self):
        ...
        with patch.dict(os.environ, {
            "CLAUDE_PLUGIN_DATA": "/Users/x/.claude/plugins/data/plugin-orchestrator-renfordn-plugins"
        }):
            sibling = path_resolution.get_sibling_plugin_data_dir(
                "plugin-orchestrator", "agent-isdd"
            )
        self.assertEqual(
            sibling,
    ...
    def test_sibling_dir_is_never_equal_to_own_dir(self):
        ...
        """
        own_dir = "/Users/x/.claude/plugins/data/plugin-orchestrator-renfordn-plugins"
        with patch.dict(os.environ, {"CLAUDE_PLUGIN_DATA": own_dir}):
            own = path_resolution.get_plugin_data_dir("plugin-orchestrator")
            sibling = path_resolution.get_sibling_plugin_data_dir(
                "plugin-orchestrator", "agent-isdd"
            )
    ...
    def test_bare_plugin_name_shape_with_no_marketplace_suffix(self):
        ...
        with patch.dict(os.environ, {
            "CLAUDE_PLUGIN_DATA": "/Users/x/.claude/plugins/data/plugin-orchestrator"
        }):
            sibling = path_resolution.get_sibling_plugin_data_dir(
                "plugin-orchestrator", "agent-isdd"
            )
    ...
    def test_hook_state_symlinks_to_the_real_sibling_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            orch_dir = os.path.join(data_root, "plugin-orchestrator-renfordn-plugins")
